...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const clearLogButton = document.getElementById('clearLog'); // Clear log button

    // Load stored resources and logs from localStorage
    const storedResources = JSON.parse(localStorage.getItem('resources')) || [];
    const actionLogs = JSON.parse(localStorage.getItem('logs')) || [];

    // Render the stored resources and logs on page load
    ...
    function renderResource(resource) {
        ...
        let resourceInfo = `<strong>${resource.name} <br>(ID: ${resource.userId})</strong> Allocated: ${resource.count} x ${resource.type} <br>at ${allocationDate.toLocaleString()}`;
